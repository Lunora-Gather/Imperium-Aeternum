...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  startScenario: (id) => { pendingScenario = id; pendingSeed = Math.floor(Math.random() * 1e9); const scenario = findScenario(id); if (!scenario.needsNationPick) { const regions = id === 'w7_random' ? [...RANDOM_SCENARIO_REGIONS[pendingSeed % RANDOM_SCENARIO_REGIONS.length]] : undefined; const state = id === 'classic' || id === 'challenge_survival' ? createInitialState() : createWorldState(pendingSeed, undefined, regions); const chosen = Object.values(state.nations).find((n) => n.isPlayer) ?? Object.values(state.nations)[0]; if (chosen) markPlayer(state, chosen.id); if (id === 'challenge_survival') applyChallenge(state); set({ state: prepareGameState(state), scene: 'playing', log: [`新游戏开始：${scenario.name} · 第 1 年`], logSeq: get().logSeq + 1, justProcessedTurn: false }); pendingScenario = null; } else set({ log: [`已选剧本：${scenario.name}，请选择你的邦国`], logSeq: get().logSeq + 1 }); },
  startWithNation: (nationId) => { if (!pendingScenario) return; const scenario = SCENARIOS.find((s) => s.id === pendingScenario); const state = createWorldState(pendingSeed, nationId, scenario?.regionFilter); const player = state.nations[nationId] ?? Object.values(state.nations)[0]; if (player) markPlayer(state, player.id); set({ state: prepareGameState(state), scene: 'playing', log: [`新游戏开始：${scenario?.name ?? pendingScenario} · 你执掌 ${player?.name ?? '未知'} · 第 1 年`], logSeq: get().logSeq + 1, justProcessedTurn: false }); pendingScenario = null; },
