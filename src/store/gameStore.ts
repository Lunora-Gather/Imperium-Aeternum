...
export const useGameStore = create<GameStore>((set, get) => ({
  state: createInitialState(), log: [], logSeq: 0, justProcessedTurn: false, scene: 'menu', returnTab: null, setReturnTab: (t) => set({ returnTab: t }), pendingProvinceId: null, setPendingProvince: (id) => set({ pendingProvinceId: id }), pendingTab: null, jumpToTab: (tab) => set({ pendingTab: tab }), consumePendingTab: () => set({ pendingTab: null }),
  startScenario: (id) => { pendingScenario = id; pendingSeed = Math.floor(Math.random() * 1e9); const scenario = findScenario(id); if (!scenario.needsNationPick) { const regions = id === 'w7_random' ? [...RANDOM_SCENARIO_REGIONS[pendingSeed % RANDOM_SCENARIO_REGIONS.length]] : undefined; const state = id === 'classic' || id === 'challenge_survival' ? createInitialState() : createWorldState(pendingSeed, undefined, regions); state.scenarioId = id; const chosen = Object.values(state.nations).find((n) => n.isPlayer) ?? Object.values(state.nations)[0]; if (chosen) markPlayer(state, chosen.id); if (id === 'challenge_survival') applyChallenge(state); set({ state: prepareGameState(state), scene: 'playing', log: [`新游戏开始：${scenario.name} · 第 1 年`], logSeq: get().logSeq + 1, justProcessedTurn: false }); pendingScenario = null; } else set({ log: [`已选剧本：${scenario.name}，请选择你的邦国`], logSeq: get().logSeq + 1 }); },
  startWithNation: (nationId) => { if (!pendingScenario) return; const scenario = SCENARIOS.find((s) => s.id === pendingScenario); const state = createWorldState(pendingSeed, nationId, scenario?.regionFilter); state.scenarioId = pendingScenario; const player = state.nations[nationId] ?? Object.values(state.nations)[0]; if (player) markPlayer(state, player.id); set({ state: prepareGameState(state), scene: 'playing', log: [`新游戏开始：${scenario?.name ?? pendingScenario} · 你执掌 ${player?.name ?? '未知'} · 第 1 年`], logSeq: get().logSeq + 1, justProcessedTurn: false }); pendingScenario = null; },
  startSharedWorld: (state, nationId, worldName) => { markPlayer(state, nationId); set({ state: prepareGameState(state), scene: 'playing', log: [`已进入共享版图：${worldName} · 你执掌 ${state.nations[nationId]?.name ?? nationId}`], logSeq: get().logSeq + 1, justProcessedTurn: false }); },
  backToMenu: () => { pendingScenario = null; useSharedWorldSessionStore.getState().leave(); set({ scene: 'menu', log: [], pendingTab: null, pendingProvinceId: null }); }, newGame: () => { useSharedWorldSessionStore.getState().leave(); get().startScenario('classic'); }, load: () => { useSharedWorldSessionStore.getState().leave(); const s = loadGame(); if (!s) return false; set({ state: prepareGameState(s), scene: 'playing', log: ['读档成功'], logSeq: get().logSeq + 1, justProcessedTurn: false }); return true; }, save: () => { if (useSharedWorldSessionStore.getState().session) { log(get, '共享版图由服务器自动保存，无需手动存档'); return; } const r = saveGame(get().state); if (r.ok) queueCloudSync(0); log(get, r.ok ? `已存档${r.sizeKB ? `（${r.sizeKB}KB）` : ''}` : (r.error ?? '存档失败')); }, clearSave: () => { deleteSave(); log(get, '已删档'); }, hasSave: () => hasSave(), saveToSlot: (slot) => { const r = saveGameToSlot(get().state, slot); if (r.ok) queueCloudSync(slot); log(get, r.ok ? `${slot === 0 ? '自动存档' : `已存档到槽位 ${slot}`}${r.sizeKB ? `（${r.sizeKB}KB）` : ''}` : (r.error ?? '存档失败')); }, loadFromSlot: (slot) => { useSharedWorldSessionStore.getState().leave(); const s = loadGameFromSlot(slot); if (!s) { log(get, '读档失败：存档不存在或损坏'); return false; } set({ state: prepareGameState(s), scene: 'playing', log: [`已读取槽位 ${slot} 存档`], logSeq: get().logSeq + 1 }); return true; }, deleteSlotSave: (slot) => { deleteSlot(slot); log(get, `已删除槽位 ${slot} 存档`); },
  nextTurn: () => { const cur = get().state; const session = useSharedWorldSessionStore.getState().session; if (session) { if (cur.pendingEvents.some((pending) => pending.nationId === cur.playerNationId)) { log(get, '请先处理待决事件'); return null; } void import('./sharedWorldStore').then(({ useSharedWorldStore }) => useSharedWorldStore.getState().setReady()).then((state) => { if (state) set({ state: prepareGameState(state), justProcessedTurn: true }); }); log(get, '已提交本年准备状态，等待统一结算'); return null; } if (cur.victory.type) return null; if (cur.pendingEvents.some((pending) => pending.nationId === cur.playerNationId)) { log(get, '请先处理待决事件'); return null; } const result = advanceTurnPipeline(cur); set({ state: result.state, justProcessedTurn: true }); if (result.report.warnings.length) log(get, result.report.warnings.join('; ')); result.notes.forEach((note) => log(get, note)); log(get, `进入第 ${result.state.turn + 1} 年`); if (result.state.victory.type?.startsWith('win') || (result.state.turn > 0 && result.state.turn % 10 === 0)) { autoSave(result.state); queueCloudSync(0); } return result.report; },
  continueLegacy: () => { const current = get().state; if (!current.victory.type?.startsWith('win') || useSharedWorldSessionStore.getState().session) return false; const state = prepareGameState({ ...current, victory: { type: null }, legacyMode: true }); set({ state, justProcessedTurn: false }); autoSave(state); queueCloudSync(0); log(get, '已进入传世经营，不再重复判定胜利'); return true; },
  clearTurnFlag: () => set({ justProcessedTurn: false }), setStrategyFocus: (focus) => { const state = get().state; if ((state.strategyFocus ?? 'balance') === focus) return; const next = { ...state, strategyFocus: focus }; commitAction(get, set, { ok: true, state: next, messages: [`国策焦点改为：${FOCUS_LABELS[focus]}`] }, { commandType: 'domestic_action', action: 'set_strategy_focus', args: [focus] }); },
  ...
  logMsg: (msg) => set((s) => ({ log: [...s.log.slice(-30), msg], logSeq: s.logSeq + 1 })),
}));
