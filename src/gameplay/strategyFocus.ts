// 战略焦点层：玩家国策 + AI 长期战略 + AI 记忆
// 1) 玩家国策：总览页右侧国策按钮继续生效。
// 2) AI 战略：各国每回合形成扩张、贸易、修复、防御、研发等长期倾向。
// 3) AI 记忆：宿敌、贸易伙伴、关注目标会长期累积，外交页可读。
...
function scoreTargets(state: GameState, nation: Nation) {
  ...
  return neighbors.map((id) => {
    const target = state.nations[id]; const rel = relationOf(state, nation.id, id);
    ...
    return {
      ...
      rival: (war ? 80 : 0) + threat * 0.8 + Math.max(0, -relation) * 0.7 + weakness * 12,
      partner: (rel?.treaty === 'trade' ? 45 : 0) + (rel?.treaty === 'alliance' ? 55 : 0) + Math.max(0, relation) * 0.7 + trust * 0.4,
...
function updateAIMemory(state: StrategicState, nation: Nation): AIMemoryEntry {
  ...
  const scored = scoreTargets(state, nation);
  const rival = scored.sort((a, b) => b.rival - a.rival)[0];
  const partner = scored.sort((a, b) => b.partner - a.partner)[0];
  const watch = scored.sort((a, b) => b.watch - a.watch)[0];
  const next: AIMemoryEntry = {
    rivalId: rival && rival.rival >= 35 ? rival.id : old.rivalId,
    rivalScore: clamp(Math.max((old.rivalScore ?? 0) * 0.72, rival?.rival ?? 0), 0, 100),
    partnerId: partner && partner.partner >= 45 ? partner.id : old.partnerId,
    partnerScore: clamp(Math.max((old.partnerScore ?? 0) * 0.76, partner?.partner ?? 0), 0, 100),
    watchId: watch && watch.watch >= 30 ? watch.id : old.watchId,
    watchScore: clamp(Math.max((old.watchScore ?? 0) * 0.78, watch?.watch ?? 0), 0, 100),
    lastUpdated: state.turn,
  };
  state.aiMemory[nation.id] = next; return next;
}

function chooseAITarget(state: StrategicState, nation: Nation, focus: AIFocusKind, memory?: AIMemoryEntry): string | undefined {
  const scored = scoreTargets(state, nation); if (scored.length === 0) return undefined;
  if ((focus === 'expansion' || focus === 'defense') && memory?.rivalId && state.nations[memory.rivalId] && !state.nations[memory.rivalId].defeated) return memory.rivalId;
  if ((focus === 'trade' || focus === 'research') && memory?.partnerId && state.nations[memory.partnerId] && !state.nations[memory.partnerId].defeated) return memory.partnerId;
  if (focus === 'trade' || focus === 'research') return scored.sort((a, b) => b.partner - a.partner)[0]?.id;
  if (focus === 'defense') return scored.sort((a, b) => b.watch - a.watch)[0]?.id;
  if (focus === 'expansion') return scored.sort((a, b) => b.rival - a.rival)[0]?.id;
  return scored.sort((a, b) => b.partner - a.partner)[0]?.id;
}

function decideAIFocus(state: StrategicState, nation: Nation, memory?: AIMemoryEntry): AIFocusKind {
  const owned = provincesOf(state, nation.id); const atWar = state.wars.some((w) => w.attackerId === nation.id || w.defenderId === nation.id);
  ...
  if (atWar || nation.warExhaustion > 45 || (memory?.watchScore ?? 0) > 70) return 'defense';
  if (nation.character === 'commerce' || nation.character === 'maritime' || nation.government.type === 'merchant_republic' || (memory?.partnerScore ?? 0) > 75) return 'trade';
  if (nation.character === 'scholarly' || nation.tech.admin + nation.tech.agri + nation.tech.mil + nation.tech.culture < 8) return 'research';
  if (nation.character === 'militarism' || nation.character === 'expansionist' || armySize(nation) > Math.max(120, owned.length * 70) || (memory?.rivalScore ?? 0) > 65) return 'expansion';
  if (nation.tier === 'S' || nation.tier === 'A') return 'expansion';
  return 'trade';
}

function reinforceBorder(state: GameState, nation: Nation): void { const border = provincesOf(state, nation.id).find((p) => p.adjacent.some((id) => state.provinces[id] && state.provinces[id].ownerId !== nation.id)); if (border) border.garrison = clamp(Math.round(border.garrison + 8), 0, 600); }
function applyAIFocusEffect(state: StrategicState, nation: Nation, entry: AIFocusEntry): void {
  const target = entry.targetId; const intensity = clamp(entry.intensity, 1, 6);
  if (entry.kind === 'recovery') { nation.government.stability = clamp(nation.government.stability + 1); nation.government.corruption = clamp(nation.government.corruption - 0.5); const troubled = provincesOf(state, nation.id).sort((a, b) => (b.unrest + b.rebellionRisk) - (a.unrest + a.rebellionRisk))[0]; if (troubled) { troubled.unrest = clamp(troubled.unrest - 3); troubled.rebellionRisk = clamp(troubled.rebellionRisk - 2); } if (nation.tier === 'C' || nation.tier === 'D') nation.resources.gold += 4; return; }
  if (entry.kind === 'defense') { nation.warExhaustion = clamp(nation.warExhaustion - 1); reinforceBorder(state, nation); if (target) for (const r of bothRelations(state, nation.id, target)) r.threat = clamp(r.threat + intensity); return; }
  if (entry.kind === 'trade') { nation.resources.influence = clamp(nation.resources.influence + 2, 0, 9999); if (target) for (const r of bothRelations(state, nation.id, target)) if (r.treaty !== 'war' && r.treaty !== 'truce') { r.relation = clamp(r.relation + 2, -100, 100); r.trust = clamp(r.trust + 1); if (r.relation >= 55 && r.trust >= 45 && r.treaty === 'none') r.treaty = 'trade'; } return; }
  if (entry.kind === 'research') { nation.resources.sciPt = clamp(nation.resources.sciPt + 4 + Math.floor(nation.tech.admin / 2), 0, 99999); nation.resources.influence = clamp(nation.resources.influence + 1, 0, 9999); if (target) for (const r of bothRelations(state, nation.id, target)) if (r.treaty !== 'war') r.trust = clamp(r.trust + 1); return; }
  nation.resources.supply = clamp(nation.resources.supply + 3, 0, 99999); reinforceBorder(state, nation); if (target) for (const r of bothRelations(state, nation.id, target)) if (r.treaty !== 'alliance' && r.treaty !== 'trade') { r.threat = clamp(r.threat + 2 + intensity); r.relation = clamp(r.relation - 1 - Math.floor(intensity / 3), -100, 100); }
}
...
function applyAIStrategy(state: GameState): GameState {
  ...
  for (const nation of Object.values(next.nations)) {
    ...
    const changed = !current || current.kind !== focus || current.targetId !== target; const intensity = changed ? 1 : clamp((current?.intensity ?? 1) + 1, 1, 6);
  ...
  for (const msg of messages.slice(0, 5)) appendWorldEvent(next, msg); next._relMap = undefined; return next;
}

export function getAIStrategyInfo(state: GameState, nationId: string): AIStrategyInfo | null {
  const s = state as StrategicState; const entry = s.aiStrategyMeta?.[nationId]; const mem = s.aiMemory?.[nationId]; if (!entry && !mem) return null;
  return { kind: entry?.kind ?? 'trade', label: entry ? AI_LABEL[entry.kind] : '观察', targetId: entry?.targetId, targetName: entry?.targetId ? state.nations[entry.targetId]?.name : undefined, intensity: entry?.intensity ?? 0, sinceTurn: entry?.sinceTurn ?? mem?.lastUpdated ?? state.turn, rivalId: mem?.rivalId, rivalName: mem?.rivalId ? state.nations[mem.rivalId]?.name : undefined, rivalScore: mem?.rivalScore, partnerId: mem?.partnerId, partnerName: mem?.partnerId ? state.nations[mem.partnerId]?.name : undefined, partnerScore: mem?.partnerScore, watchId: mem?.watchId, watchName: mem?.watchId ? state.nations[mem.watchId]?.name : undefined, watchScore: mem?.watchScore };
}

export function installStrategyFocus(): void {
  if (installed) return; installed = true; const originalNextTurn = (useGameStore.getState() as unknown as { nextTurn?: () => unknown }).nextTurn;
  useGameStore.setState({ setStrategyFocus: (id: StrategyFocusId) => { const cur = useGameStore.getState() as unknown as { state: GameState; logMsg?: (msg: string) => void }; const old = ((cur.state as StrategicState).strategyFocus ?? 'balance') as StrategyFocusId; if (old === id) return; useGameStore.setState({ state: { ...cur.state, strategyFocus: id } as GameState } as never); const label: Record<StrategyFocusId, string> = { balance: '均衡', stability: '安民', prosperity: '富国', military: '强军', diplomacy: '睦邻', reform: '改革' }; cur.logMsg?.(`国策焦点改为：${label[id]}`); }, nextTurn: () => { const result = originalNextTurn?.(); const cur = useGameStore.getState() as unknown as { state: GameState; logMsg?: (msg: string) => void }; if (cur.state.victory.type) return result; const focus = (((cur.state as StrategicState).strategyFocus) ?? 'balance') as StrategyFocusId; const playerApplied = applyPlayerFocus(cur.state, focus); let next = playerApplied.state; next = applyAIStrategy(next); useGameStore.setState({ state: next } as never); if (playerApplied.note) cur.logMsg?.(playerApplied.note); return result; } } as never);
}
