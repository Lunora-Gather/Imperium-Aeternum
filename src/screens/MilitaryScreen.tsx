...
import MilitaryWarOpportunityBridge from '../components/MilitaryWarOpportunityBridge';
            ...
            {targets.map((p) => {
              ...
              return (
                <div key={p.id} className="ia-card" style={{ padding: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div><strong style={{ fontSize: 12 }}>{p.name}</strong><span className="dim" style={{ fontSize: 10, marginLeft: 6 }}>{isHub ? '首都枢纽' : '相邻'}</span></div>
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div onClick={(e) => e.stopPropagation()} className="ia-battle-in" style={{ background: 'var(--bg-panel)', borderRadius: 10, padding: 18, maxWidth: 560, width: '90%', border: '2px solid var(--war)', boxShadow: '0 8px 32px rgba(0,0,0,0.5), 0 0 24px rgba(162,61,40,0.15)', maxHeight: '80vh', overflow: 'auto' }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-mute)', marginBottom: 12 }}>{attacker?.name} 攻 vs {defender?.name} 守 · 进度 {Math.round(war.progress)}% · 已 {war.turns} 年</div>
        {war.battleReports.length === 0 ? <p className="dim">尚无战报</p> :
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {war.battleReports.slice().reverse().map((r, i) => {
              ...
              return <div key={i} className="ia-card ia-report-row" style={{ padding: 10, borderLeft: `3px solid var(--${tone === 'good' ? 'good' : tone === 'danger' ? 'war' : 'warn'})`, animationDelay: `${i * 0.08}s` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}><strong style={{ fontSize: 12, color: `var(--${tone === 'good' ? 'good' : tone === 'danger' ? 'war' : 'warn'})` }}>{icon} {r.narrative}</strong></div>
                <div style={{ fontSize: 11, color: 'var(--text-dim)', display: 'flex', gap: 12 }}><span>我军 {r.attSize} → 折 {r.attLoss}</span><span>敌军 {r.defSize} → 折 {r.defLoss}</span><span>进度 {r.progressDelta >= 0 ? '+' : ''}{r.progressDelta}%</span></div>
              </div>;
            })}
          </div>}
      </div>
...
export default function MilitaryScreen() {
  ...
  return <div>
    <Panel title="军情判断" accent>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 8 }}>
        {guidance.slice(0, 3).map((g, i) => <div key={i} className="ia-card" style={{ padding: 10, borderLeft: `3px solid var(--${g.tone === 'danger' ? 'war' : g.tone === 'warn' ? 'warn' : g.tone === 'good' ? 'good' : 'border'})` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}><strong style={{ fontSize: 13 }}>{g.title}</strong><Tag text={g.tone === 'danger' ? '紧急' : g.tone === 'warn' ? '注意' : g.tone === 'good' ? '机会' : '情报'} tone={g.tone} /></div>
    ...
    <Panel title={`军队部署（${player.army.length}）`}>
      {player.army.length === 0 ? <span className="dim">无军队。在省份或下方征兵。</span> : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 8 }}>
        {player.army.map((a) => {
          const prov = state.provinces[a.location];
          const front = frontierProvinceIds.has(a.location);
          return <div key={a.id} className="ia-card" style={{ padding: 10, borderLeft: front ? '3px solid var(--war)' : undefined }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}><strong style={{ fontSize: 13 }}>{prov?.name ?? a.location}</strong><div style={{ display: 'flex', gap: 4 }}><Tag text={`${a.size}人`} tone="info" />{front && <Tag text="前线" tone="danger" />}</div></div>
            <StatRowMini label="士气" value={a.morale} kind="high" /><StatRowMini label="训练" value={a.training} kind="high" /><StatRowMini label="装备" value={a.equipment} kind="high" />
    ...
    <Panel title={`进行中的战争（${myWars.length}）`}>
      {myWars.length === 0 ? <span className="dim">无</span> : myWars.map((w) => {
    ...
    <Panel title="宣战">
      ...
      <div className="ia-card" style={{ marginBottom: 8, padding: 8, background: 'rgba(201,120,40,0.08)', border: '1px solid var(--warn)' }}><span style={{ fontSize: 11, color: 'var(--warn)' }}>⚠ 战略要点：宣战后须在“军队部署”区把军队调动到与敌省相邻的己省（前线），否则无军队在前线将自动议和。</span></div>
      ...
      <div>{possibleWarRows.map(({ p, adj, rel }) => {
        if (rel?.treaty === 'truce' && rel.truceTurns > 0) {
          const enemy = state.nations[adj.ownerId];
          return <div key={`${p.id}-${adj.id}`} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 60px', gap: 8, padding: '8px 0', borderBottom: '1px solid var(--border)', alignItems: 'center', opacity: 0.6 }}><span style={{ fontSize: 12 }}>邻省 <strong>{adj.name}</strong> <span className="dim">属 {enemy?.name ?? adj.ownerId}</span></span><span style={{ fontSize: 12, color: 'var(--warn)' }}>停战 {rel.truceTurns} 回合</span><Tag text="停战" tone="warn" /></div>;
        }
