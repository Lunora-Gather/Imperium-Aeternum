...
export default function ScenarioSelect() {
  ...
  return <div className="ia-menu ia-menu--launch">
    ...
    <header className="ia-launch-hero">
      ...
      <button className="ia-featured-scenario" onClick={() => startPickedScenario(featuredScenario.id)}>
        <div className="ia-featured-top">
          ...
          <Tag text={featuredHint.marketTag} tone={featuredHint.tone} />
        </div>
        <h2 className="ia-display">{featuredScenario.name}</h2>
        <p>{featuredScenario.description}</p>
        <div className="ia-featured-meta">
          <span>{featuredScenario.subtitle}</span>
          <span>{featuredChallenge.label}</span>
          <span>压力 {featuredChallenge.pressure}</span>
        </div>
        <strong>{featuredChallenge.headline}</strong>
        <em>{featuredScenario.needsNationPick ? '选择邦国后开始' : '立即开始'}</em>
      </button>
    </header>
    ...
    <div className="ia-launch-guide-grid"><Guide title="第一次玩" body="选地中海黎明，最快理解财政、省份、战争和事件。" tone="good" /><Guide title="喜欢经营" body="选印度洋贸易，经济和海贸路线更清晰。" tone="info" /><Guide title="喜欢冲突" body="选地中海争霸，战争、贸易和外交都很密集。" tone="warn" /><Guide title="追求旗舰长局" body="选万邦纪元，但建议熟悉系统后再玩。" tone="danger" /></div>
    <div id="scenario-library" className="ia-scenario-library-head"><div><span className="ia-up">Campaign Library</span><h2 className="ia-display">选择剧本</h2></div><p>按压力和玩法定位选择，不同剧本对应不同学习曲线。</p></div>
    <div className="ia-scenario-grid">{SCENARIOS.map((s) => { const hint = getScenarioProfile(s.id); const challenge = buildScenarioChallengeGuide(s.id); const isRecommended = recommended.has(s.id); return <button key={s.id} className={`ia-scenario-card ${isRecommended ? 'is-recommended' : ''}`} onClick={() => startPickedScenario(s.id)} style={{ borderColor: isRecommended ? `var(--${toneVar(hint.tone)})` : undefined }}><div className="ia-scenario-card__top"><h3 className="ia-display">{s.name}</h3><Tag text={s.nationCount} tone="gold" /></div><div className="ia-scenario-sub">{s.subtitle}</div><p>{s.description}</p><div className="ia-scenario-tags"><Tag text={hint.marketTag} tone={hint.tone} /><Tag text={challenge.label} tone={challenge.tone} /><Tag text={`压力 ${challenge.pressure}`} tone={challenge.tone} />{isRecommended && <Tag text="推荐" tone="gold" />}</div><div className="ia-scenario-advice">{challenge.headline} · {challenge.recommendedAfter}</div><div className="ia-scenario-foot">{s.needsNationPick ? '选择邦国' : '开始剧本'}<span>→</span></div></button>; })}</div>
...
function HandbookPanel({ handbook }: { handbook: LaunchHandbook }) {
  return <section className="ia-launch-handbook">
    <div className="ia-handbook-head">
      <div>
        <Tag text="玩法速查" tone="gold" />
        <strong>{handbook.headline}</strong>
        <p>{handbook.subtitle}</p>
      </div>
      <Tag text={handbook.primaryAdvice} tone="good" />
    </div>
    <div className="ia-handbook-grid">
      {handbook.sections.map((section) => <article key={section.id} className={`ia-handbook-item ia-handbook-item--${section.tone}`}>
        ...
        <p>{section.summary}</p>
        <div>{section.steps.slice(0, 3).map((x) => <Tag key={x.id} text={x.label} tone={x.tone} />)}</div>
      </article>)}
    </div>
  </section>;
}
...
function ChallengePanel({ guide, compact = false }: { guide: ScenarioChallengeGuide; compact?: boolean }) {
  return <section className={`ia-challenge-panel ${compact ? 'ia-challenge-panel--compact' : ''}`} style={{ borderColor: `var(--${toneVar(guide.tone)})` }}>
    <div className="ia-challenge-head">
      <div>
        <Tag text={guide.label} tone={guide.tone} />
        <strong>{guide.headline}</strong>
      </div>
      <Tag text={`压力 ${guide.pressure}/100`} tone={guide.tone} />
    </div>
    <p>{guide.why}</p>
    {!compact && <em>推荐前置：{guide.recommendedAfter} · 常见失败：{guide.likelyFailure}</em>}
    <div className="ia-challenge-tags">{guide.checklist.map((x) => <Tag key={x.id} text={x.text} tone={x.tone} />)}</div>
  </section>;
}
    ...
    <div className="ia-difficulty-steps">
      {path.map((id, index) => {
        ...
        return <button key={id} className="ia-difficulty-step" onClick={() => onPick(id)}>
          <span>第 {index + 1} 阶</span>
          <strong>{s?.name ?? id}</strong>
          <em>{guide.label} · {guide.pressure}</em>
        </button>;
      })}
    </div>
  </section>;
  ...
  return <section className="ia-continue-panel" style={{ borderColor: `var(--${toneVar(summary.tone)})` }}>
    <div className="ia-continue-copy">
      <div className="ia-continue-tags">
        <Tag text="继续游戏" tone={summary.tone} />
      ...
      <strong>{best ? `推荐继续：槽位 ${best.slot} · ${best.nationName ?? '未知国家'} · 第 ${(best.turn ?? 0) + 1} 年` : summary.headline}</strong>
    ...
    <div className="ia-continue-actions">
      ...
      {!best && <Btn label="开始推荐剧本" variant="primary" onClick={onStartRecommended} />}
      <Btn label="刷新体检" variant="ghost" onClick={onRefresh} />
    </div>
  </section>;
}
