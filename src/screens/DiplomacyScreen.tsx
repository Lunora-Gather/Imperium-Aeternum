// Diplomacy v3 — 外交判断：威胁、贸易、同盟、AI 意图先于列表
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', padding: 8 }}>
      <svg width={400} height={400} viewBox="0 0 400 400" style={{ maxWidth: '100%' }}>
        ...
        <g fontSize={9} fill="var(--text-mute)"><circle cx={12} cy={12} r={5} fill="var(--gold)" /><text x={22} y={15}>同盟</text><circle cx={60} cy={12} r={5} fill="var(--good)" /><text x={70} y={15}>贸易</text><circle cx={108} cy={12} r={5} fill="var(--war)" /><text x={118} y={15}>战争</text><circle cx={156} cy={12} r={5} fill="var(--bg-inset)" stroke="var(--border)" /><text x={166} y={15}>中立</text></g>
      </svg>
    </div>
  );
}
...
export default function DiplomacyScreen() {
  ...
  const rows = otherNations.map((n) => ({ n, rel: findRelationExplicit(pid, n.id, state), ai: getAIStrategyInfo(state, n.id) })).filter((x): x is { n: Nation; rel: DiplomaticRelation; ai: ReturnType<typeof getAIStrategyInfo> } => !!x.rel);
  ...
  const threats = [...rows].filter((x) => x.rel.treaty === 'war' || x.rel.threat >= 45 || x.rel.relation <= -45 || x.ai?.kind === 'expansion').sort((a, b) => (b.rel.threat + (b.ai?.kind === 'expansion' ? 35 : 0)) - (a.rel.threat + (a.ai?.kind === 'expansion' ? 35 : 0))).slice(0, 3);
  const tradeTargets = [...rows].filter((x) => x.rel.treaty === 'none' && x.rel.relation >= 0 && player.resources.influence >= 30).sort((a, b) => b.rel.relation + b.rel.trust - (a.rel.relation + a.rel.trust)).slice(0, 3);
  ...
  const strategicAdvice = threats.length > 0 ? `优先关注 ${threats[0].n.name}：${threats[0].rel.treaty === 'war' ? '已处战争' : threats[0].ai?.kind === 'expansion' ? '有扩张意图' : '威胁偏高'}。` : tradeTargets.length > 0 ? `可先与 ${tradeTargets[0].n.name} 建立贸易，增加长期收益。` : allianceTargets.length > 0 ? `可考虑与 ${allianceTargets[0].n.name} 结盟，稳定周边。` : '当前外交格局较平稳，可积累影响力等待机会。';
  ...
  return (
    <div>
      ...
      {view === 'list' && <Panel title="各国关系"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 10 }}>{sortedRows.map(({ n, rel, ai }) => { const relTone = rel.relation < -30 ? 'danger' : rel.relation < 30 ? 'warn' : 'good'; const targetText = ai?.targetName ? ` → ${ai.targetName}` : ''; return <div key={n.id} id={`diplo-${n.id}`} className="ia-card" style={{ padding: 12, border: focusNation === n.id ? '2px solid var(--gold)' : rel.treaty === 'war' ? '1px solid var(--war)' : undefined, transition: 'border 0.3s' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><strong style={{ fontSize: 14 }}>{n.name}</strong><div style={{ fontSize: 11, color: 'var(--text-mute)' }}>{n.government.type} · {n.character} · {n.tier}级</div></div><div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>{ai && <Tag text={`${ai.label}${targetText}`} tone={intentTone(ai.kind)} />}<Tag text={treatyText(rel)} tone={TREATY_TONE[rel.treaty] ?? 'info'} /></div></div>{ai && <div style={{ fontSize: 10, color: 'var(--text-dim)', marginBottom: 6 }}>AI 意图：{ai.label} · 强度 {ai.intensity}/6 · 起于第 {ai.sinceTurn + 1} 年</div>}<RelBar label="关系" value={rel.relation} tone={relTone} /><RelBar label="信任" value={rel.trust} tone={rel.trust > 50 ? 'good' : rel.trust > 20 ? 'warn' : 'danger'} /><RelBar label="威胁" value={rel.threat} tone={rel.threat > 50 ? 'danger' : rel.threat > 20 ? 'warn' : 'good'} invert /><div style={{ display: 'flex', gap: 4, marginTop: 10, flexWrap: 'wrap' }}><Btn label="改善 20影" variant="ghost" onClick={() => improveRelation(n.id)} disabled={player.resources.influence < 20} /><Btn label="贸易 30影" variant="ghost" onClick={() => formTrade(n.id)} disabled={player.resources.influence < 30 || rel.relation < 0 || rel.treaty === 'war'} /><Btn label="同盟 50影" variant="ghost" onClick={() => formAlliance(n.id)} disabled={player.resources.influence < 50 || rel.relation < 50 || rel.treaty === 'war'} /><Btn label="联姻 30影80金" variant="ghost" onClick={() => dynasticMarriage(n.id)} disabled={player.resources.influence < 30 || player.resources.gold < 80 || rel.relation < 20 || rel.treaty === 'war'} /><Btn label="文化输出 30科" variant="ghost" onClick={() => culturalExport(n.id)} disabled={player.resources.sciPt < 30 || rel.treaty === 'war'} /><Btn label={intelTarget === n.id ? '✦情报' : '情报 40影'} variant={intelTarget === n.id ? 'primary' : 'ghost'} onClick={() => setIntelTarget(intelTarget === n.id ? null : n.id)} disabled={player.resources.influence < 40 || rel.treaty === 'alliance'} /></div>{intelTarget === n.id && <div className="ia-fade-in" style={{ marginTop: 8, padding: 8, background: 'var(--bg-inset)', borderRadius: 6, border: '1px solid var(--warn)' }}><div style={{ fontSize: 11, color: 'var(--warn)', fontWeight: 700, marginBottom: 6 }}>情报行动 · 会影响双方信任</div><div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}><Btn label="科技情报" variant="ghost" onClick={() => { espionage(n.id, 'steal_tech'); setIntelTarget(null); }} /><Btn label="地方扰动" variant="ghost" onClick={() => { espionage(n.id, 'foment_rebellion'); setIntelTarget(null); }} /><Btn label="军情观察" variant="ghost" onClick={() => { espionage(n.id, 'spy_military'); setIntelTarget(null); }} /></div></div>}</div>; })}</div></Panel>}
    </div>
  );
}

function MiniList({ title, rows, empty, tone, onPick }: { title: string; rows: { n: Nation; rel: DiplomaticRelation; ai: ReturnType<typeof getAIStrategyInfo> }[]; empty: string; tone: 'danger' | 'warn' | 'good' | 'info'; onPick: (id: string) => void }) {
