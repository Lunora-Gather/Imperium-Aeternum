...
export default function SaveLoadScreen() {
  const { saveToSlot, loadFromSlot, deleteSlotSave, state, log } = useGameStore();
  ...
  return localizeReactTree(<div>
    ...
    <Panel title="存档管理" accent>
      ...
      <Divider label="操作" />
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}><CampaignExitControl presentation="save" /><Btn label="刷新体检" variant="ghost" onClick={refresh} /><Btn label="清空全部存档" warn onClick={doClearAll} /></div>
