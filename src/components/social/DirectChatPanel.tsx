...
export function DirectChatPanel({ friendUserId, friendName, onBack }: { friendUserId: string; friendName: string; onBack: () => void }) {
  ...
  const sending = Boolean(store.sending[`direct:${friendUserId}`]);
  ...
  const submit = async () => {
    ...
    else if (!attachment) setBody((current) => current || draft);
  };
  ...
  return <section className="ia-direct-chat">
    ...
    <form className="ia-world-chat-compose ia-direct-chat-compose" onSubmit={(event) => { event.preventDefault(); void submit(); }}><textarea className="ia-input" aria-label={t('给 {{name}} 的消息', { name: friendName })} value={body} maxLength={attachment ? 300 : 500} placeholder={t(attachment ? '给图片附带一句说明（可选）' : '发送消息给 {{name}}', { name: friendName })} onChange={(event) => setBody(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey && !attachment) { event.preventDefault(); void submit(); } }} /><div><input ref={fileRef} hidden type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => setAttachment(event.target.files?.[0] ?? null)} /><button className="ia-chat-attach-btn" type="button" disabled={sending} onClick={() => fileRef.current?.click()}>▧ {t('图片')}</button><span>{body.length}/{attachment ? 300 : 500}</span><Btn type="submit" label={t(attachment ? '发送图片' : '发送')} variant="primary" busy={sending} disabled={sending || (!attachment && !body.trim())} /></div></form>
  </section>;
}
