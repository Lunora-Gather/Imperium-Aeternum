...
export function PlayerProfileCard({ profile, relation, isSelf = false, onAdd, onChat, onClose }: {
  ...
  return <div className="ia-profile-backdrop" onClick={(event) => { event.stopPropagation(); onClose(); }}>
    <section className="ia-player-card ia-fade-in" role="dialog" aria-modal="true" aria-label={t('{{name}} 的玩家名片', { name: profile.displayName })} onClick={(event) => event.stopPropagation()}>
      <button className="ia-modal-close" onClick={onClose} aria-label={t('关闭玩家名片')}>×</button>
      <div className={`ia-player-card-cover tone-${profile.avatarColor}`}><span>IMPERIUM IDENTITY</span></div>
      <div className="ia-player-card-identity">
        <ProfileAvatar profile={profile} size="large" />
        <div><Tag text={t(profile.title)} tone="gold" /><h2 className="ia-display">{profile.displayName}</h2><p>{t(seen.key, { count: seen.count ?? 0 })}</p></div>
      </div>
      <blockquote>{t(profile.bio)}</blockquote>
      <div className="ia-player-card-meta">
        <div><span>{t('好友码')}</span><strong>{profile.friendCode}</strong></div>
        <div><span>{t('加入纪元')}</span><strong>{new Date(profile.createdAt).toLocaleDateString()}</strong></div>
      </div>
      <footer>
