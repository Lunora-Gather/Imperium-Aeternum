...
export function WorldChatPanel({ worldId, nationId }: { worldId: string; nationId?: string }) {
  ...
  return <section className={`ia-world-chat ${open ? 'is-open' : ''}`}>
    ...
    {open && <div className="ia-world-chat-body ia-fade-in">
      <div className="ia-world-chat-head"><div><strong>万邦会谈室</strong><span>实时消息 · 最近 50 条</span></div><em>文字与图片仅本版图成员可见</em></div>
      ...
      <div ref={logRef} className="ia-world-chat-log" aria-live="polite">{messages.length ? messages.map((item) => {
        ...
        return <article key={item.id} className={mine ? 'is-mine' : ''}><span className="ia-chat-avatar">{item.displayName.slice(0, 1).toUpperCase()}</span><div><header><strong>{mine ? '我' : item.displayName}</strong>{item.nationId && <em>{item.nationId === nationId ? '当前执政国' : '版图成员'}</em>}<time>{new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></header>{item.kind === 'image' && item.mediaFileId && <a className="ia-chat-image" href={worldMessageMediaUrl(item.mediaFileId)} target="_blank" rel="noreferrer"><img src={worldMessageMediaUrl(item.mediaFileId)} alt={item.body === '图片' ? `${item.displayName} 分享的图片` : item.body} loading="lazy" /></a>}{(item.kind !== 'image' || item.body !== '图片') && <p>{item.body}</p>}</div></article>;
