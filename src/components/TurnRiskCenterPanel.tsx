...
export default function TurnRiskCenterPanel({ plan, jumpToPrimary }: { plan: TurnRiskCenterPlan; jumpToPrimary?: () => void }) {
  ...
  return <section className="ia-dash-section" style={{ borderColor: toneBorder(plan.tone) }}>
    <header>
      <div><small>Pre-turn Risk</small><h3>推进前风险中枢</h3></div>
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
        <Tag text={plan.decision === 'block' ? '先处理' : plan.decision === 'prepare' ? '建议整备' : '可推进'} tone={tagTone(plan.tone)} />
        <Tag text={`准备 ${plan.readiness}`} tone={tagTone(plan.tone)} />
      </div>
    </header>
    ...
    <div className="ia-action-list" style={{ marginBottom: 8 }}>
      {visible.length === 0 ? <div className="ia-risk-empty">暂无风险或机会。</div> : visible.map((item, i) => <button key={`${item.id}-${i}`} className={`tone-${item.tone === 'danger' ? 'danger' : item.tone === 'warn' ? 'warn' : 'normal'}`} onClick={jumpToPrimary} disabled={!jumpToPrimary}>
        <b>{i === 0 ? `优先：${item.title}` : item.title}</b>
        <span>{item.body}</span>
      </button>)}
    </div>
    ...
    <button className="ia-card" onClick={jumpToPrimary} disabled={!jumpToPrimary} style={{ width: '100%', padding: 10, textAlign: 'left', cursor: jumpToPrimary ? 'pointer' : 'default', border: `1px solid ${toneBorder(plan.tone)}` }}>
      ...
      <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 4 }}>{plan.primaryTab ? '点击跳转到首要处理页面。' : '当前没有明确处理页面。'}</div>
    </button>
  </section>;
}
