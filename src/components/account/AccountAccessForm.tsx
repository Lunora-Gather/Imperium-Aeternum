...
export function AccountAccessForm() {
  ...
  const awaitingCode = store.pendingOtpPurpose === (mode === 'register' ? 'register' : 'login');
  ...
  if (mode === 'register') {
    ...
    return <div className="ia-auth-access">
      ...
      <div className="ia-auth-progress" aria-label="注册进度"><span className="is-done">1</span><i className={awaitingCode ? 'is-done' : ''} /><span className={awaitingCode ? 'is-current' : ''}>2</span><b>{awaitingCode ? '验证邮箱并完成注册' : '填写账号资料'}</b></div>
      {!awaitingCode ? <form className="ia-auth-form" onSubmit={(event) => { event.preventDefault(); void sendCode('register'); }}>
  ...
  const loginAwaitingCode = method === 'otp' && awaitingCode;
  return <div className="ia-auth-access">
    <AccessTabs mode={mode} onChange={switchMode} />
