...
| **C1** | 引擎纯函数化 | `engine/*.ts settleEconomy/settlePolitics/...` | 子引擎改 `(nation, state) => partialResult` 不 mutate；processTurn 合并结果建新 state；可回滚/重放 | XL | **六步已完成（v2.12→v2.17）：settleEconomy/settlePopulation/settlePolitics/settleTechnology/settleCultureReligion/settleDiplomacy 六子引擎 Pure 版本 + 对照测试验证等价 + 不 mutate（101/101 全绿）；剩余 settleWars + applyEffect/recordEvent/ageRulers/activateTendency + processTurn 迁移留下回合** |
