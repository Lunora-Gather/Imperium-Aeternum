...
| ~~C1~~ | ~~引擎纯函数化~~ | ~~`engine/*.ts settleEconomy/settlePolitics/...`~~ | ~~**完成（v2.12→v2.20）：10 子引擎 Pure 版本（settleEconomy/Population/Politics/Technology/CultureReligion/Diplomacy/Wars/applyEffect/recordEvent/ageRulers + lawPerTurnEffects）+ processTurnPure 渐进式迁移（6 子引擎 Pure 收集 deltas 合并 + 6 保留原版本 mutate next）+ 16 对照测试验证等价（10 子引擎 × 1 + processTurnPure × 6 含 50 回合长回合对照）+ 116/116 全绿 + 50 回合 ~1.8s 无 NaN。activateTendency 已纯只读。剩余 processAITurn 纯函数化留下回合（三层分层 mutate 范围广）**~~ | XL→完成 |
...
**Phase C 验收门槛**：~~C4 收尾完成~~ ✅ + ~~C3 完成~~ ✅ + ~~C2 完成~~ ✅ + ~~C1 完成~~ ✅，引擎可纯函数重放、116 测试、操作响应 <10ms、50 回合 ~1.8s。**当前：C1/C2/C3/C4/C5 全部 ✅，Phase C 完整交付。**
...
| ~~D2~~ | ~~科技质变效果~~ | ~~`data/technologies.ts` + `formulas.ts`~~ | ~~**完成（v2.4→v2.5）：32 科技 4 路线×8 级 + Lv5/Lv8 解锁能力绑定（agri_lv5 轮作政策/mil_lv8 总动员/admin_lv5 科举法/culture_lv5 文化外交等），通过 prereqTech 字段复用现有机制无需新代码**~~ | 需 DEC 解锁"科技不超 3×8"红线到质变 | L→完成 |
...
> - v2.12→v2.15：**完成 C1 第二/三/四步（三个子引擎纯函数化，零回归）**。settlePopulationPure（popDelta/classSatDelta/factionSatFinal，avgSat 用 classSatNew 算保持等价）/settlePoliticsPure（govFinal 覆写值，临时变量 finalStab 等算保持等价）/settleTechnologyPure（deltaSciPt/researchProgressFinal/techLevelUp）。各 +2 对照测试验证等价 + 不 mutate（91→97 全绿）。**教训：纯函数版 mutate 依赖点需用临时变量算 final 保持语义等价——settlePolitics 的 corruptionDelta 依赖 stability mutate 后值，纯函数版用 finalStab 判断。**
> - v2.15→v2.17：**完成 C1 第五/六步（两个子引擎纯函数化，零回归）**。settleCultureReligionPure（provFinal 每省 assimilation/loyalty/rebellionRisk 覆写值，loyalty 用 finalAssim 算保持等价）/settleDiplomacyPure（relationsFinal/nationsGovFinal/newChronicle，两遍遍历——第一遍算 relation final，第二遍用 relationsFinal.threat 判联军反制等价原函数 mutate 后值）。各 +2 对照测试验证等价 + 不 mutate（97→101 全绿）。**教训：settleDiplomacy 联军反制依赖 relation mutate 后的 threat 值，纯函数版两遍遍历——第一遍算 final 存 relationsFinal，第二遍用 relationsFinal.threat 判断，保持语义等价。**
> - v2.17→v2.18：**完成 C1 全部 10 子引擎 Pure 基础设施（零回归 111/111）**。settleWarsPure（XL 最复杂：wars splice + battleReports + armies size/morale + provs ownerId/garrison/assimilation/loyalty/unrest + nations resources/warExhaustion/stability/defeated + relations treaty/truceTurns/relation/trust + chronicle + atWar，applyPeaceEffects 辅助函数收集 peace 后 effects）/applyEffectPure（nation resources/gov/warExhaustion/taxRate + relations + factions + provs.population + pendingEvents）/recordEventPure（triggeredEvents + cooldowns）/ageRulersPure（ruler.age/reignYears/heir + 继承处理 + gov.legitimacy）/lawPerTurnEffectsPure（provs unrest/rebellionRisk）。各 +2 对照测试验证等价 + 不 mutate（101→111 全绿）。activateTendency 已是纯只读函数无需 Pure。processTurnPure 骨架建立（复用原 processTurn 零回归保险），完整迁移留下回合（需用正确字段名重写合并逻辑）。**教训：XL 子引擎纯函数化（settleWars 含 makePeace splice）需用辅助函数 applyPeaceEffects 收集 peace 后 effects（割让/赔款/孤儿撤退/关系改 truce/编年史/atWar 清除），避免原函数 splice 后遍历逻辑断裂；Pure 子引擎返回类型字段名需先盘点（delta/popDelta/classSatDelta/factionSatFinal/govFinal/deltaSciPt/deltaGold/researchProgressFinal/techLevelUp/provFinal/relationsFinal/nationsGovFinal/newChronicle）再写合并逻辑——避免字段名错误返工。**
> - v2.18→v2.19：**完成 C1 最后一步——processTurnPure 渐进式迁移（零回归 114/114）**。processTurnPure 完整实现：调用 6 子引擎 Pure（settleEconomy/Population/Politics/Technology/CultureReligion/Diplomacy）收集 deltas 合并到 next（economy delta 全增量含 adminPt/sciPt 覆写转 delta / population popDelta+classSatDelta 合并到 p.classes[].satisfaction / politics govFinal 覆写 / technology deltaSciPt+deltaGold+researchProgressFinal+techLevelUp 合并 / cultureReligion provFinal 覆写 / diplomacy relationsFinal+nationsGovFinal+newChronicle 合并），保留 6 子引擎原版本 mutate next（settleWars/processAITurn/ageRulers/applyEffect/recordEvent/lawPerTurnEffects——mutate 范围广或与 AI/事件交织深）。+3 对照测试验证与原 processTurn 等价（player resources/gov/provinces 关键字段 + report 字段一致，同种子）。111→114 全绿。**教训：processTurnPure 渐进式迁移策略——先迁移易合并的 6 子引擎（返回结构化 deltas/finals），保留难合并的 6 子引擎原版本（settleWars 含 splice/processAITurn 三层分层/ageRulers 继承随机/applyEffect 多类型效果），零回归铺路；对照测试用同种子验证 next state 关键字段一致（toBeCloseTo 容浮点误差）。**
> - v2.19→v2.20：**C1 完整交付 + Phase C 全部完成（零回归 116/116）**。processTurnPure 50 回合对照测试验证长回合下与原 processTurn 等价（player resources/gov/provinces 关键字段一致，同种子）+ 推进 50 回合无 NaN/崩溃。114→116 全绿。C1 状态行更新为~~完成~~（10 子引擎 Pure + processTurnPure 渐进式迁移 + 16 对照测试），Phase C 验收门槛全部 ✅（C1/C2/C3/C4/C5）。**教训：长回合对照测试（50 回合）是验证渐进式迁移等价性的关键——单回合对照可能漏掉累积误差（浮点误差累积/状态漂移），50 回合对照确保长程等价。C1 完整交付方法论：并存版本 → 对照测试验证等价 → 渐进式迁移 → 长回合对照 → 零回归。**
