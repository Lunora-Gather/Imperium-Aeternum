...
## DEC-046：共享活版图采用服务端权威世界与独立玩家会话

- **阶段**：多人共享世界基础（2026-07-22）
- **背景**：目标允许不同用户加入不同版图、同账号控制多个国家、禁止重复认领；只要版图有人操作，其余国家仍由 AI 推进。同时需要同版图聊天与全局好友。
- **决策**：共享 `WorldState` 不保存用户身份，控制列表与当前视角归入独立会话；控制权使用唯一租约行，敏感写入全部经过 Appwrite Functions；版图只有一个年份和修订，由统一时钟编排在线玩家、离线托管和无主 AI。GitHub Pages 继续托管静态前端，Appwrite 承担 Auth、TablesDB、Functions、Storage 与 Realtime。
- **影响**：现有单机与云存档保持兼容；共享世界需要新增命令日志和服务端 Resolver，不能把个人云存档直接开放为多人状态。好友和聊天作为独立社交域，不进入模拟引擎。
- **替代**：把 `playerNationId` 改为数组（否决：用户身份污染公共世界）；浏览器直接更新共享存档（否决：无法防并发覆盖和作弊）；立即迁移 Vercel（否决：静态前端不是当前限制，增加运维面但不解决权威结算）。

## DEC-047：新账号以邮箱 OTP 验证作为注册提交边界

- **阶段**：可信账号与社交体验（2026-07-22）
- **背景**：共享版图中的控制权、聊天和好友关系需要稳定身份；直接创建邮箱密码账号会留下未验证但可登录的身份。
- **决策**：新注册先使用 Appwrite Email OTP 建立已验证会话，再设置昵称和密码；密码登录必须检查 `emailVerification`。验证码完全由 Appwrite 生成与验证。
- **影响**：注册增加一个明确验证步骤；游客单机不受影响。旧的未验证会话会被拒绝，可通过邮箱验证码完成验证。
- **替代**：自建验证码表和发信函数（否决：扩大敏感数据与运维面）；仅发送验证链接但允许继续登录（否决：无法满足注册必须验证的产品约束）。

## DEC-048：共享版图采用“活跃窗口 + 定时截止”统一推进

- **阶段**：共享版图可靠性强化（2026-07-23）
- **背景**：只靠所有玩家点击“准备完毕”会让离线玩家永久阻塞版图；只靠客户端点击又会允许人为加速世界。
- **决策**：保留全员准备后的即时结算，同时为权威网关配置每 5 分钟的 Appwrite 定时执行。只有已初始化、处于计划阶段、到达截止时间且当前计划窗口内有真实成员活动的版图才结算；无人参与时暂停。结算前事务化释放到期租约，所有扫描使用游标分页。
- **影响**：有人参与的版图不会因其他玩家离线而停摆；空世界不会后台无限增长；未来增加版图数量时不受 100 行首屏限制。
- **替代**：浏览器心跳直接推进（否决：可被点击频率操纵）；所有版图无条件定时推进（否决：产生无人游玩的无意义历史）；常驻 Vercel 服务（否决：Appwrite 定时函数已覆盖当前需求）。

## DEC-049：验证码找回密码必须经过新鲜 OTP 会话网关

- **阶段**：账号恢复修复（2026-07-23）
- **背景**：Appwrite 普通 `updatePassword` 对已有密码账号要求旧密码，OTP 登录后直接调用仍会报凭据错误；客户端也不应持有管理员改密能力。
- **决策**：Email OTP 继续负责 6 位验证码与会话建立；独立 Account Gateway 使用调用者 JWT 读取当前会话，只接受 10 分钟内新建、provider 为 token 且含 email factor 的会话，并由动态 API key 仅更新同一用户的既有已验证密码账号。
- **影响**：保留用户要求的 6 位验证码体验，同时修复“验证码正确但仍提示邮箱或密码不正确”；旧会话、密码会话和跨用户请求均不能绕过校验。
- **替代**：改成邮件跳转链接（安全但不符合验证码产品要求）；任意登录会话均可后台改密（否决：会放大会话窃取风险）；自建验证码（否决：重复 Appwrite 已有认证能力）。
