...
**冻结说明**：此结构一旦确认，后续阶段只能新增文件，不得随意移动已有文件。改动需记入 `docs/reference/decisions.md`。
...
|------|------|------|--------------|-----------------|
| 1 | 设计圣经 | `docs/reference/01-design-bible.md` | 文档评审 checklist | high |
| 2 | 系统规则 + 公式总表 | `docs/reference/02-system-rules.md`、`docs/reference/formulas.md` | 公式可手算验算 | high |
| 3a | 核心数据表（国家/省份/建筑/科技/政体） | `docs/reference/03-data-tables.md` 上半 + 对应 `src/data/*.ts` | `tsc --noEmit` 通过 | medium |
| 3b | 内容数据表（事件/政策/派系/AI/国家性格） | `docs/reference/03-data-tables.md` 下半 + 对应 `src/data/*.ts` | `tsc --noEmit` 通过、数据自检脚本 | medium |
| 4 | 类型 + 架构 + engine 骨架 + 公式实现 | `docs/reference/04-architecture.md` + `src/types` + `src/engine/formulas.ts` + 单测 | `npm test` 公式单测全绿 | high |
...
| 6 | 扩展设计 | `docs/planning/06-expansion.md` | 文档 | medium |
...
### B. 关键决策记录（写入 docs/reference/decisions.md）
...
1. **每阶段完成后，产出文件即为冻结基线**。文件头部加：
   ...
   ```
2. **后续阶段如需推翻冻结内容**，必须在 `docs/reference/decisions.md` 写一条 ADR：
   ```markdown
   ## DEC-007 将省份从 12 个减为 10 个
   - 背景：5a 实现时发现 12 省份 UI 过密
   - 决策：减为 10 个
   - 影响：需回改 src/data/provinces.ts、docs/reference/03-data-tables.md
   - 替代：保留 12 但加分页（否决：增加复杂度）
   ```
3. **代码生成时，优先引用冻结文档的具体小节号**，而非凭记忆重述。例如 prompt：
   > "实现 `economy.ts`，公式严格按 `docs/reference/formulas.md §2.1` 税收公式，不得自创。"
...
- 公式命名严格对应 `docs/reference/formulas.md` 的小节号（如 `§2.1` → `computeTax`），便于人工核对。
...
冻结基线：
- docs/reference/01-design-bible.md (FROZEN v1)
- docs/reference/02-system-rules.md (FROZEN v1)
- docs/reference/formulas.md (FROZEN v1)
- ...
...
A. 交付物清单（文件路径）
B. 关键决策记录（写入 docs/reference/decisions.md）
C. 验证清单（见 docs/planning/00-project-plan.md §3.2）

约束：
- 不得修改冻结文档，需改走 DEC-NNN
- 公式严格引用 docs/reference/formulas.md 小节号
...
下一步应将以下内容作为给 GLM-5.2 的第 1 阶段启动指令（本文件已作为 `docs/planning/00-project-plan.md` 冻结基线存在）：

```
你是 Imperium Aeternum 的首席策划 + 系统设计师 + 架构师。

请阅读并严格遵守 docs/planning/00-project-plan.md（已冻结）。其中：
- §1 项目结构
- §2 阶段划分
- §3 产出契约
- §4 冻结点机制
...
产出：
- 文件：docs/reference/01-design-bible.md
- 内容按 §3.2 阶段 1 验证清单组织，必须包含以下小节：
  ...
  9. 下一阶段要产出的数据表预告
- 关键决策写入 docs/reference/decisions.md（DEC-001 起）
...
> **FROZEN v1** — 本文件为项目总规划基线。后续阶段引用本文不得修改；如需调整规划本身，走 `docs/reference/decisions.md` DEC-NNN。
