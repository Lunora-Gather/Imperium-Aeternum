...
| applyEffect | applyEffectPure | `ApplyEffectPureResult { resourceDeltas, warExhaustionFinal, taxRateFinal, assimilationModFinal, govFinal, relationOverrides, factionSatFinals, provPopDeltas, newPendingEvents }` | ✅（正式可变 API 已成为纯结果应用器的薄包装） |
| recordEvent | recordEventPure | `RecordEventPureResult { newTriggeredEntry, cooldownUpdate }` | ✅（记录与冷却均按国家隔离） |
