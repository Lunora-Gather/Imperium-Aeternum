...
版图为空且 `pauseWhenEmpty=true` 时停止计划结算。在所有受控国家提交“本年准备完毕”后立即结算；若未全部准备，Appwrite 每 5 分钟检查一次到期版图。只有当前计划窗口内确实有成员活动的版图会被定时推进，无人参与的版图继续暂停。无人控制国家在结算中执行完整 AI 行动，受控国家执行被动年度结算但不会被 AI 擅自下达高风险命令。任何国家不得拥有独立于版图的年份。

到期租约会在结算前以事务重新校验并释放，随后该国重新进入 `ai_autonomous`。定时扫描、控制权、成员与命令列表均使用游标分页，避免多版图或大版图只处理前 100 条。

进入治理、提交行动和确认准备都会刷新控制租约与最后活动时间，持续活跃的玩家不会因最初认领日超过 14 天而被误释放。同一国家同一回合的“准备完毕”由服务端生成唯一幂等键，重复点击不会堆积命令；过期控制者不计入当前回合必需准备人数。

续期只能作用于调用者仍持有且尚未过期的控制权，不能代替认领流程或绕过每人控制国上限。国家在同一回合换手后，新控制者必须亲自重新确认准备，旧控制者的准备记录不会被继承。
...
首版聊天室最多向 100 名成员配置行级读取权限；第 101 名成员会被网关明确拒绝，而不是静默截断消息或媒体权限。超过此规模前必须迁移为每版图 Appwrite Team 或专用聊天服务。
