...
async function currentRow(db, userId, slot) {
  try { return await db.getRow({ databaseId: DATABASE_ID, tableId: SAVE_TABLE, rowId: buildCloudSaveRowId(userId, slot) }); }
...
async function uploadSave(db, storage, userId, body) {
  ...
  try {
    const row = await db.upsertRow({ databaseId: DATABASE_ID, tableId: SAVE_TABLE, rowId: buildCloudSaveRowId(userId, slot), data: { userId, slot, fileId: uploaded.$id, saveVersion, turn, nationName, deviceId, contentHash, clientUpdatedAt }, permissions: access });
    if (previous?.fileId && previous.fileId !== uploaded.$id) await storage.deleteFile({ bucketId: SAVE_BUCKET, fileId: previous.fileId }).catch(() => undefined);
    return row;
  } catch (error) {
